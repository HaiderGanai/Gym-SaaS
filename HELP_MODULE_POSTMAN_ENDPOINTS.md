...
All three endpoints are `GET`, **public** — no `Authorization` header, no
...
shapes will keep working once real FAQ/legal copy replaces the constants.
...
- No request body, no query params, no auth on any of the three endpoints.
- Content is intentionally hardcoded (`ponytail:` marked in `help.service.ts`)
  — swap the `FAQS` / `PRIVACY_POLICY` / `TERMS` constants for real copy
  whenever legal/product delivers it. No route or response-shape change
  needed on the frontend when that happens.
