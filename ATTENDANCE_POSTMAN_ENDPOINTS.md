...
**Same three fields on pause/resume:** `PATCH /subscriptions/:id/pause`, `PATCH /subscriptions/:id/resume` (staff), and their member self-service equivalents `PATCH /subscriptions/me/:id/pause` / `PATCH /subscriptions/me/:id/resume` all return the single updated subscription object with `total_days`/`days_left`/`check_ins` included — same values and freeze behavior as above, computed at the moment the pause/resume takes effect. A pause response shows `days_left`/`check_ins` already frozen; a resume response shows them ticking again from the shifted `current_period_end`.
