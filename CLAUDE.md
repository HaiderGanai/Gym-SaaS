...
| POST | `/notifications/test-push` | MemberJwt | `{ title?, body? }` — fires a canned notification through every channel (FCM + web push) the member has registered; 404 if none registered; doesn't write to the in-app inbox |
